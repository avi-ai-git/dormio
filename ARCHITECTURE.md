...
your message
   ...
   -> router node (intent, any cities, a country, and an operator)
...
A small LangGraph with a router, two tool nodes, and a synthesis node. The router reads the latest message and the recent thread, decides between route, knowledge, both, and chitchat, and pulls out any cities, a country, or an operator. With a model it uses a short few-shot prompt that returns JSON. Without a model, or under test, it falls back to a deterministic heuristic that reads the text around the word to and checks each candidate city against the graph, so the pipeline runs with no keys at all.
...
A golden set of 28 questions, each tagged with the tool it should reach and the outcome it should produce, scores whether the router picks the right tool, whether the graph returns the right kind of result including the country and operator modes, and whether retrieval finds the right guide. A judge flag adds groundedness, an automated check that each answer is supported by the facts the tools returned. The latest numbers are routing 100 percent live and 96 percent offline, route correctness 100 percent, retrieval 90 percent, and groundedness around 4 out of 5. Alongside the eval, 73 offline tests cover the graph, retrieval, the agent, safety, data integrity, and the failure paths.
